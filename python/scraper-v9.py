...
#
# Run:
#   python ailabwatch_full_scraper_v9.py
#
# Output:
...
async def parse_category_page(page: Page, slug: str) -> List[Dict]:
    ...
    # JS: extract per-company scores within region between this <h2> and the next <h2>
    extract_scores_js = """
    (h2, args) => {
      const { categorySlug, companySlugMap } = args;

      // Collect the region nodes
      const regionNodes = [];
      let n = h2.nextElementSibling;
      while (n && n.tagName !== 'H2') { regionNodes.push(n); n = n.nextElementSibling; }

      // Clone into a detached region for reliable querying
      ...
      for (const company of Object.keys(companySlugMap)) {
        const cslug = companySlugMap[company];
        let val = null;

        // Find anchors to /cell/<company>/<category>
        const anchors = region.querySelectorAll(`a[href*="/cell/${cslug}/${categorySlug}"]`);
        for (const a of anchors) {
          // climb to cardish container
          let container = a;
    ...
    for i in range(count):
        ...
        # Subcategory weight (evaluate on the element)
        weight = await header.evaluate(get_weight_js)

        # Scores (evaluate on the element; pass args as a single object)
        scores = await header.evaluate(
            extract_scores_js,
            {"categorySlug": slug, "companySlugMap": COMPANY_SLUG},
        )
